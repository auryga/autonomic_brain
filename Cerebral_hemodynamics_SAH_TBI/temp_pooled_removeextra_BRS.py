...
dictionary_list =['HRV_RMSSD', 'BRS', 'HR', 'HRhfRel', 'HRV_HF','HRV_LF', 'HRV_LFHF', 'HRV_TP', 'HRV_SDSD','HRlfDivhtNorm',  'HRlfDivhtRel', 'HRhfN', 'abp', 'Mx_L', 'Mx_R', 'FVR', 'FVL']
...
for n in range(0,files_number):
    ...
    if side_patient == 2:
        ...
        if np.isnan(FV).any() == True:
            results_final['FV_ipsi'].values[n] = np.nansum(FV)
        else:
