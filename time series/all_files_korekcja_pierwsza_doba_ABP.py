...
SOURCE_FILE_PATH = "C:\Moje_dokumenty\Po_doktoracie_09_2021\Miniatura\wyniki_17_1\BRS_results"
EXTENSION_FILE = ".csv"
...
abp_av12_values_iteration = pd.DataFrame(index = np.arange(files_number), columns =np.arange(26))
...
for counter_files in range(0,files_number):
    ...
    filled_a = interpolate_gaps(dataset['ABP'], limit=2)
    
    
    dataset2 = pd.DataFrame(filled_i)
    dataset3 = pd.DataFrame(filled_b)
    dataset4 = pd.DataFrame(filled_a)
    frames = [dataset2,dataset3,dataset4]
    result = pd.concat(frames,axis=1)
    result.columns = ['ICP', 'BRS','ABP']
    ...
    ###############korelacja w dniach - TOTAL bez 1 doby
    if korekcja['korekcja'].loc[counter_files] == 0:
        
        BRS_total_bez1 = result['BRS'].iloc[360*24+1:-1]
        ICP_total_bez1 = result['ICP'].iloc[360*24+1:-1]
        if len(dataset) > 360*24:
        
            r = BRS_total_bez1.corr(ICP_total_bez1, method = 'spearman')
            ...
            r = np.nan
            results_iteration['r_total'].loc[counter_files] = np.nan
    ...
    #rs1 =   brs_first_day.corr(icp_first_day)
        ...
        rs1 = np.nan
        results_iteration['r_1'].loc[counter_files] = np.nan
    ...
    if korekcja['korekcja'].loc[counter_files] == 0:
        ...
        if len(dataset) > 360*24:
            ...
            results_iteration['offset_total'].loc[counter_files] = np.nan
            
    else:
            rs = [crosscorr(result['BRS'],result['ICP'], lag) for lag in range(0,180+1)]
            offset = np.floor(len(rs)/2)-np.argmax(rs)
            offset_minutes = offset/6
    ...
    if korekcja['korekcja'].loc[counter_files] == 0:
        ...
        results_iteration['offset_1'].loc[counter_files] = np.nan
    ...
    for i in range (0, len(result),window):
        ...
        abp_av12.append(np.nanmean(result['ABP'].iloc[i:i+window]))
    ...
    plt. savefig('Z'+only_files_names[counter_files]+".jpg")
    ...
    abp_av12_values_iteration.iloc[counter_files,0:len(abp_av12)] = abp_av12
...
abp_av12_korekcja = pd.DataFrame(index = np.arange(files_number), columns =np.arange(40))
abp_av12_korekcja.iloc[:,len(abp_av12_values_iteration)] = abp_av12_values_iteration
...
for counter_files in range(0,files_number):
    ...
    if korekcja['korekcja'].loc[counter_files]!=0:
        ...
        zj = abp_av12_values_iteration.iloc[counter_files,0:len(abp_av12_values_iteration.columns)]
        zj = zj.tolist()
        abp_av12_korekcja.iloc[counter_files,(korekcja_aktualna*2):(len(abp_av12_values_iteration.columns)+korekcja_aktualna*2)] = zj
        ...
        zj = abp_av12_values_iteration.iloc[counter_files,0:len(abp_av12_values_iteration.columns)]
        zj = zj.tolist()
        abp_av12_korekcja.iloc[counter_files,0:len(zj)] =  zj
...
abp_av12_korekcja.to_csv(SAVE_FILE_PATH + '/' +'abp_av12_korekcja.csv', sep = ';', index = False)
