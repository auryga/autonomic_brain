...
SOURCE_FILE_PATH2 = "C:\Moje_dokumenty\Po_doktoracie_09_2021\Miniatura\wyniki_17_1\prx_amp"
...
files_list2, only_files_names2 = get_list_files(SOURCE_FILE_PATH2,EXTENSION_FILE)
...
for counter_files in range(0,files_number):
    ...
    dataset22 = read_file(files_list2[counter_files], separator = ';', decimal_sign = ',')
    dataset22.drop(['DateTime'], axis=1, inplace=True)
    #drop of outliers
    dataset22=outliers(dataset22,3)
    ...
    filled_p = interpolate_gaps(dataset22['PRx'], limit=2)
    
    
    dataset2 = pd.DataFrame(filled_i)
    dataset3 = pd.DataFrame(filled_b)
    dataset4 = pd.DataFrame(filled_p)
    
    frames = [dataset2,dataset3, dataset4]
    result = pd.concat(frames,axis=1)
    result.columns = ['ICP', 'BRS','PRx']
    ...
    result['mov_av_PRx'] = result['PRx'].rolling(window=720,center=True,min_periods=1).median()
    ...
    fig,ax = plt.subplots(figsize=(20,10))
    # make a plot
    ax.plot(a_k,result['mov_av_PRx'], color="red", label = 'ICP')
    # set x-axis label
    ax.set_xlabel("time [days]")
    plt.gca().set_xlim(right=4)
    # set y-axis label
    ax.set_ylabel("PRx [a.u]",color = "red")
    #plt.axline((0, 0), (-1, 0),color="red",linestyle = 'dashdot')
    #ax.set(ylim = [-1,1])
    #plt.xlim(left=0)
    ...
    plt.savefig('0PRXtotal_days'+only_files_names2[counter_files]+".jpg")
    ...
    plt.savefig('0total_days'+only_files_names[counter_files]+".jpg")
